...
# Agregar el directorio raíz al PYTHONPATH para poder importar backend.rag
...
def run_full_rebuild(year_limit=None):
    ...
    # 1. Limpieza selectiva o total de la base de datos
    if year_limit:
        ...
        # Purgar registros del año específico antes de reindexar para evitar duplicados
    ...
    # 2. Identificar años en la carpeta de escalabilidad
    if year_limit:
    ...
    if not years:
        print(f"[!] No se encontraron carpetas de años para procesar.")
        return
    ...
    for year in years:
        ...
        # Usamos la lógica de carga de RAGPipeline pero solo para este año
        from backend import rag as rag_module
        old_data_path = rag_module.DATA_PATH
        rag_module.DATA_PATH = year_dir
        
        try:
            chunks = rag.load_and_split_documents()
            
            if chunks:
                print(f"[*] Año {year}: {len(chunks)} fragmentos generados. Integrando en la DB...")
                
                # SUB-LOTES: Si un año tiene muchos fragmentos, los metemos de 1000 en 1000
                sub_batch_size = 1000
        ...
        rag_module.DATA_PATH = old_data_path
